...
def extract_response(api_response):
    """
    Extracts text from various types of model responses, excluding the initial
    {'role': 'user', 'content': '...'} part from the predictions.
    """
    extracted_messages = []
    ...
    if isinstance(api_response, list):
        ...
        for prediction in api_response['predictions']:
            # Use regex to remove the initial part
...
def update_output_and_typewriter(n_clicks, selected_model_endpoint, input_text, max_tokens, temperature, chat_history):
    ...
    # Setup additional parameters if provided
    kwargs = {}
    ...
    try:
        if isinstance(response, list) and response:
            chat_history.append({'sender': 'model', 'text': response[0]})
            typewriter_text = response[0]  # Set the text for the typewriter effect
        else:
            chat_history.append({'sender': 'model', 'text': "Unexpected response format received."})
    except Exception as e:
        chat_history.append({'sender': 'model', 'text': f"Error processing model response: {str(e)}"})

    # Save chat history to a file after update
